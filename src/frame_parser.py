...
def gen_pwr_state_name(value):
    return f'{PwrState(value).name}::{value}'

def gen_pwr_status_name(value):
    return f'{PwrStatus(value).name}::{value}'

def gen_diesel_state(value):
    return f'{DieselState(value).name}::{value}'

def gen_load_states(value):
    return f'{LdStates(value).name}::{value}'

def gen_dpf_state(value):
    return f'{DpfState(value).name}::{value}'

def gen_tmc_request(value):
    return f'{TmcRequest(value).name}::{value}'


def parse_tmc_status(payload):
    data= {'tmc_state': payload[0] & 0x0F,
...
def parse_tmc_status2(payload):
    data= {'RTT_timestamp': payload[2] | (payload[3] << 8), # 3-4 N/A Timestamp (sec)
...
def parse_tmc_status5(payload):
    ...
    data['pwr_finished_flag_STATE_1'] = finished_flags==1
    data['pwr_finished_flag_STATE_2'] = finished_flags==2
    data['pwr_finished_flag_STATE_3'] = finished_flags==3
    data['pwr_finished_flag_STATE_4'] = finished_flags==4
    data['pwr_finished_flag_STATE_5'] = finished_flags==5
    data['pwr_finished_flag_STATE_6'] = finished_flags==6
    data['pwr_finished_flag_STATE_7'] = finished_flags==7
    data['pwr_finished_flag_STATE_8'] = finished_flags==8

    
    return data

def parse_tmc2acu(channel, payload):
    data = {f'tmc_req_{channel}_s': gen_tmc_request((payload[0] >> 0) & 0x0F), # 1.1-1.4 N/A TMC Request
...
def parse_acu_status(channel, payload):
    data = {f'diesel_state_{channel}': (payload[0] >> 4) & 0x0F, # 1.5-1.8 Diesel State
            f'diesel_state_{channel}_s': gen_diesel_state((payload[0] >> 4) & 0x0F),
            f'dpf_state_{channel}_s': gen_dpf_state(payload[1] & 0x0F),
...
def parse_tmc_status3(payload):
    ...
    for i in range(4):
        ...
        data[f'PowerSource_Status_{i+1}_s'] = gen_pwr_status_name(payload[i] & 0x07)
        data[f'PowerControl_State_{i+1}']  = (payload[i] >> 3) & 0x07
        data[f'PowerControl_State_{i+1}_s']  = gen_pwr_state_name((payload[i] >> 3) & 0x07)
...
def parse_tmc_status4(payload):
    ...
    for i in range(4):
        ...
        data[f'Charger_State_{i+1}_s'] = gen_load_states(payload[i] & 0x07)
        ...
        data[f'Cooler_State_{i+1}_s'] = gen_load_states(payload[i+4] & 0x07)
